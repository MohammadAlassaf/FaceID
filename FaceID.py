...
def capture_images(num_images, output_folder):
    ...
    for i in range(num_images):
        ...
        # Break the loop if 'q' is pressed (or Arabic layout 'ض'/'ö' code 214, or ESC key 27)
        k_cap = cv2.waitKey(1) & 0xFF
        if k_cap == ord('q') or k_cap == 214 or k_cap == 27:
            break
...
def set_face_id():
    ...
    anchor = tf.data.Dataset.list_files(os.path.join(ANC_PAS, '*.jpg')).take(100)
    positive = tf.data.Dataset.list_files(os.path.join(POS_PAS, '*.jpg')).take(100)
    negative = tf.data.Dataset.list_files(os.path.join(NEG_PAS, '*.jpg')).take(100)
